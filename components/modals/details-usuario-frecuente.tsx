...
interface DetailsUsuarioFrecuenteModalProps {
    ...
    onRechazar?: (usuarioFrecuente: UsuarioFrecuente) => void
}
                ...
                <Dialog.DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="grid grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            {renderFilePreview(
                                getVal(usuarioFrecuente?.imagenes, ["Certificado de Estudios", "Certificado"]) || usuarioFrecuente?.imagen_certificado,
                                `Certificado de ${usuarioFrecuente?.nombre}`,
                                () => {
                                    const src = getVal(usuarioFrecuente?.imagenes, ["Certificado de Estudios", "Certificado"]) || usuarioFrecuente?.imagen_certificado;
                                    if (src) handleFileClick(src, `Certificado de ${usuarioFrecuente?.nombre}`);
                                }
                            )}
                        </div>
                        ...
                        {usuarioFrecuente?.imagenes && Object.entries(usuarioFrecuente.imagenes).map(([key, src]) => {
                            ...
                            return (
                                <div key={key}>
                                    ...
                                    {renderFilePreview(
                                        src,
                                        `${key} de ${usuarioFrecuente?.nombre}`,
                                        () => handleFileClick(src, `${key} de ${usuarioFrecuente?.nombre}`)
                                    )}
                                </div>
                            );
                        })}
                    ...
                    <div className="flex justify-end gap-2 border-t pt-4 mt-2">
                        ...
                        {usuarioFrecuente && onRechazar && usuarioFrecuente.status !== "RECHAZADO" && (
                            <Button
                                variant="outline"
                                className="text-destructive hover:bg-destructive/10"
                                onClick={() => onRechazar(usuarioFrecuente)}
                            >
                                <XCircle className="mr-2 h-4 w-4" /> Rechazar
                            </Button>
                        )}
                        {usuarioFrecuente && onToggleStatus && (
                            <Button
                                variant={usuarioFrecuente.status === "ACTIVO" ? "destructive" : "default"}
                                onClick={() => {
                                    onToggleStatus(usuarioFrecuente.id, usuarioFrecuente.status as any)
                                }}
                            >
